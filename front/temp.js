function Placeholders() {
    ...
    if (predefinidos.qualificacao_completa_do_imputado_.length > 1) {
        ...
        placeholdersToFill = placeholdersToFill.filter(ph => ph !== "qualificacao_completa_do_imputado");
        const placeholdersMultiplos = extrairPlaceholdersMultiplos(placeholders, multiAuthorPlaceholders);
  ...
  function extrairPlaceholdersMultiplos(placeholders, multiAuthorPlaceholders) {
    ...
    const selectedPiecesId = window.finalSelectedPieces;
    const requerMultiplos = selectedPiecesId.some(id => piecesSeparateIds.includes(id));
  
    if (requerMultiplos && window.predefinidos.qualificacao_completa_do_imputado_.length > 1) {
      window.predefinidos.qualificacao_completa_do_imputado_.forEach((imputado, index) => {
  ...
  function criarFormulario(listaParaInputs, predefinidos) {
    ...
    function atualizarAltura(input) {
      if (input.name.includes("cidade_onde_o_procedimento_esta_sendo_realizado")) {
    ...
    listaParaInputs.forEach((placeholder) => {
      ...
      if (placeholder.includes("qualificacao_do_condutor")) {
        ...
        input.value = predefinidos.qualificacao_da_vitima_.join(", ").toUpperCase();
